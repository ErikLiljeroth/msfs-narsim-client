...
def update_pos_msfs_aircraft(
    ...
    updated_pos = init_position(
        ...
        c_double(
            float(narsim_flight_dict["truth"]["pitch"]["#text"])
        ),  # <- pitch: added after fix from mbjork
        c_double(
            float(narsim_flight_dict["truth"]["bank"]["#text"])
        ),  # <- bank: added after fix from mbjork
...
class ClientFlightsCounter:
    ...
    def get_object_id(self, callsign: str) -> int:
        ...
        return next(
            (
                item["object_id"]
                for item in self.flight_objects
...
def traffic_injector_process(s: socket.socket, simc_injector: SimConnect) -> None:
